...
public class MeetControllerTest {
    ...
    public void getConversations() {
        ...
        when(providersService.getConversationsForUser(currentUserId)).thenReturn(Flux.fromIterable(asList(
                new ConversationDto("conversation-1", new ProfileDto("profile-id-1", "profile-name-1", 1, emptyList()), new MessageDto("message-1", now(), true, "message-text-1")),
                new ConversationDto("conversation-2", new ProfileDto("profile-id-2", "profile-name-2", 2, emptyList()), new MessageDto("message-2", now(), false, "message-text-2"))
        )));
