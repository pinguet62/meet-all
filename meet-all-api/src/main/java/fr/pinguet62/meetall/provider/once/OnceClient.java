...
class OnceClient {
    ...
    public Mono<OnceMatchAllResponseDto> getMatchsHistoryFiltered(String authorization) {
        return getMatchsHistoryFiltered(authorization, MAX_VALUE);
    }

    /**
     * @param size Default: {@code 10}
     */
    Mono<OnceMatchAllResponseDto> getMatchsHistoryFiltered(String authorization, @Nullable Integer size) {
        return webClient
                .get()
    ...
    public Mono<OnceMatchPassResponseDto> passMatch(String authorization, String matchId) {
        return passOrLikeMatch(authorization, matchId, "pass")
                .bodyToMono(OnceMatchPassResponseDto.class);
    }

    public Mono<OnceMatchLikeResponseDto> likeMatch(String authorization, String matchId) {
        return passOrLikeMatch(authorization, matchId, "like")
                .bodyToMono(OnceMatchLikeResponseDto.class);
    }

    private ResponseSpec passOrLikeMatch(String authorization, String matchId, String passOrLike) {
        return webClient
                .post()
                .uri(uriBuilder -> uriBuilder.path("/v1/match").pathSegment(matchId).pathSegment(passOrLike).build())
                .header(HEADER, authorization)
                .retrieve();
    }
